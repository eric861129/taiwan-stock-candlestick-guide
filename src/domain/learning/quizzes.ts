...
export const STAGE_QUIZZES: readonly StageQuiz[] = [
  ...
  },
  {
    ...
    questions: [
      ...
      makeQuestion(2, 4, '多時間週期閱讀的合理做法是什麼？', ['讓較大週期提供背景，再用小週期描述觸發', '只使用一分鐘圖', '只使用最大週期', '讓不同週期互相覆蓋不記錄'], 'b', '大週期提供背景，小週期可補充觸發細節。'),
  ...
  },
  {
    stageId: 'stage-3',
    questions: [
      makeQuestion(3, 1, '單根 K 線的長上影通常先描述什麼？', ['上方曾有較高成交但收回的痕跡', '公司一定虧損', '隔日一定下跌', '交易一定成功'], 'c', '影線是觀察痕跡，需搭配位置與後續確認。'),
      makeQuestion(3, 2, '雙根或三根組合的名稱之外，還要補上什麼？', ['發生位置與前後背景', '固定目標價', '保證勝率', '與市場無關的暱稱'], 'b', '同一組合在不同位置可能代表不同的觀察意義。'),
      makeQuestion(3, 3, '量價關係中的「失敗訊號」較接近哪個概念？', ['條件未被確認或後續被否定', '任何下跌都算失敗', '任何放量都算成功', '不用記錄失效'], 'a', '失敗是條件被否定或未確認，應留下可查核紀錄。'),
      makeQuestion(3, 4, '低流動性股票的型態判讀需要特別注意什麼？', ['價差與零星成交可能放大形狀', '只看圖不看成交', '忽略交易成本', '把缺值補成預測'], 'd', '低流動性會放大雜訊，應把成本與成交連續性納入。'),
      makeQuestion(3, 5, '型態卡的「常見誤讀」用途是什麼？', ['提醒名稱不能取代證據', '提供買賣指令', '取代所有章節', '保證每次結果'], 'c', '誤讀欄位協助辨識把描述誤當成結論的風險。'),
    ],
  },
  {
    stageId: 'stage-4',
    questions: [
      makeQuestion(4, 1, 'ATR 在風險筆記中比較適合作為什麼？', ['波動程度的參考尺度', '未來價格保證', '公司評價', '交易許可證'], 'd', 'ATR 可協助描述波動尺度，不能直接給出方向。'),
      makeQuestion(4, 2, '情境、觸發與失效三者的順序應如何記錄？', ['先寫情境，再列觸發與失效條件', '只寫觸發', '先寫結果再補條件', '只寫型態名稱'], 'c', '先定義背景情境，再說明何時觸發、何時失效。'),
      makeQuestion(4, 3, '部位大小與停損距離的共同目的為何？', ['把單筆風險控制在可承受範圍', '讓每筆交易都獲利', '省略交易成本', '提高預測準確率'], 'b', '風險控制是邊界管理，不是獲利保證。'),
      makeQuestion(4, 4, '什麼情況適合選擇「不交易」？', ['條件不完整或失效界線無法清楚定義', '任何看到 K 線時', '只要有成交量時', '為了完成題數時'], 'a', '無法查核條件時，放棄交易本身就是紀律。'),
      makeQuestion(4, 5, 'R 倍數與期望值主要協助檢查什麼？', ['風險報酬與長期紀錄', '公司內線消息', '保證勝率', '明日開盤價'], 'd', '它們是事後與長期紀錄工具，不是預測器。'),
    ],
  },
  {
    stageId: 'stage-5',
    questions: [
      makeQuestion(5, 1, '遮圖案例實驗的第一個原則是什麼？', ['先寫下當下可見證據', '先看完整答案', '先猜最大漲幅', '先套用最熟悉名稱'], 'b', '遮圖練習先固定當下資訊，才能檢查推理是否越界。'),
      makeQuestion(5, 2, '交易紀錄中加入「失效原因」有什麼價值？', ['讓事後回顧能對照原先條件', '保證下一次成功', '取代原始資料', '刪除不利案例'], 'd', '記錄失效原因，才能檢查條件是否需要調整。'),
      makeQuestion(5, 3, '心理偏誤檢查清單的用途是什麼？', ['辨識自己如何跳過證據或風險界線', '推測別人的持股', '提供盤中訊號', '取代學習章節'], 'c', '偏誤檢查把決策過程拉回可觀察紀錄。'),
      makeQuestion(5, 4, '綜合案例驗收最重要的輸出是什麼？', ['可查核的觀察、條件與風險紀錄', '單一預測答案', '保證獲利曲線', '最熱門的型態名稱'], 'a', '能力驗收重點是推理證據鏈與風險界線。'),
      makeQuestion(5, 5, '完成階段測驗後，章節閱讀權限如何處理？', ['所有章節仍可自由開啟', '只開放下一章', '答錯就永久鎖定', '必須登入才能重看'], 'b', '測驗只記錄學習進度，不會鎖住任何章節。'),
    ],
  },
];
