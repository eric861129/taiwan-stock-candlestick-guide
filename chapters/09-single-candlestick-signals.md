...
下表採本書用於練習的觀察慣例。市場或書籍可能有不同門檻；遇到差異時，先列出採用的 OHLC 關係，再談名稱。令實體下緣 $B_L=\min(O,C)$、實體上緣 $B_H=\max(O,C)$、當日範圍 $R=H-L$，並令 $u=\max(|C-O|, 一個適用的價格升降單位或來源四捨五入單位)$。只有 $R>0$ 時才檢查下列三分之一位置。

| 查找標籤 | 可重現的 OHLC／形狀定義 | 在解釋前仍需補足的背景 |
| --- | --- | --- |
| 十字線 | 開收近似相等；上下影線可長可短。 | 前段方向、關鍵區域、量價關係與後續收盤。 |
| 錘子形 | **整個實體**位於上三分之一：$B_L\ge L+2R/3$；下影線 $B_L-L\ge2u$；上影線 $H-B_H\le u$。 | 前段是否下行、是否靠近支撐區、是否有可接受的觸發與失效條件。 |
| 射擊之星形 | **整個實體**位於下三分之一：$B_H\le L+R/3$；上影線 $H-B_H\ge2u$；下影線 $B_L-L\le u$。 | 前段是否上行、是否靠近壓力區、是否有確認或回落證據。 |
...
![人工日 K 線圖。左側下行後出現實體靠近上方且有長下影線的錘子形，中段出現長實體與不符合錘子形條件的 K 線，右端以小實體保留為待確認；紅色空心、綠色實心與圖例之外，同時用標籤說明 OHLC 關係。](../assets/figures/ch09-concept.svg)
...
公司行動以 TWSE [除權息計算結果資料](https://www.twse.com.tw/rwd/zh/exRight/TWT49U?response=json&startDate=20240501&endDate=20240531) 查核；該期間查詢結果沒有 2330 記錄，所以 metadata 的 `corporate_actions` 為空陣列。[公開資訊觀測站「公告快易查」](https://mopsov.twse.com.tw/mops/web/ezsearch)是公司公告的交叉查核入口。沒有已查得的除權息資料，只能排除已知機械調整，不能替影線或實體指定原因。
...
<!-- figure-spec
{
  ...
  "alt_text": "台積電 2330 於 2024 年 5 月 6 日至決策日 5 月 20 日的官方原始日 K 線與成交量。圖中標示 5 月 15 日的小實體與長上影線，以及右端 5 月 20 日有長下影線但上影線超過本書錘子形門檻；圖表停在決策日，沒有後續走勢。",
  ...
  "corporate_actions": [],
  "annotations": [
    {"type": "zone", "start": "2024-05-06", "end": "2024-05-13", "low": 786, "high": 825, "label": "前段整理與上移"},
...
![台積電 2330 於 2024 年 5 月 6 日至決策日 5 月 20 日的官方原始日 K 線與成交量。圖中標示 5 月 15 日的小實體與長上影線，以及右端 5 月 20 日有長下影線但上影線超過本書錘子形門檻；圖表停在決策日，沒有後續走勢。](../assets/figures/ch09-cases.svg)

### 有效、失敗與模稜兩可

- **可用的形狀觀察**：5/15 的開收差距相對小，最高價高於實體；5/20 的實體全數位於當日範圍上三分之一且有較長下影線，但上影線為 3 元，大於本書門檻 $u=1$ 元，因此不符合本書錘子形的完整定義。這些句子都可由 OHLC 重做。
- **不成立的跳躍**：把 5/17 的長實體或 5/20 的長下影線直接改寫成「錘子形已成立」或「隔日必然轉向」，都忽略了形狀門檻、觸發、失效與可交易性資料。
...
**八步前置核對。**核對 TWSE、2330、日 K、原始價格、日期排序與公司行動查核結果；若改用還原價格，實體比較、區域與容忍值要以同一價格模式重算，較長週期另用完整週 K 檢查。

1. **大方向**：描述前段整理、上移或回落，不把一根 K 線當成完整趨勢結論。
2. **所在位置**：把 5/15、5/20 放回前段整理帶、可能的支撐區或壓力區，而非只看影線比例。
3. **量價反應**：用前 20 根完成 K 線的比較窗、開收容忍值、實體與影線關係判定形狀，並比較同一標的、同一週期量能；沒有價差與深度時，流動性保持證據不足。
4. **交易情境**：把「收盤守住區域後追蹤反彈」與「只保留形狀觀察」分開，型態名稱本身不是情境。
5. **觸發條件**：只有完成 K 線收盤守住預先定義區域，且必要量價條件已核對，才觸發追蹤。
6. **失效條件**：收盤跌回預先定義區域下方、容忍值不再符合或資料模式改變時，反彈情境失效。
7. **風險計算**：以觸發到區域下方失效價的距離估每股風險，將部位、滑價與資料品質限制一併納入。
8. **事後檢討**：保存比較窗、幾何條件與未交易理由，回顧時檢查是否把單根 K 線的名稱誤寫成方向保證。
...
**示範答案。**觀察：5/20 的 $O=834$、$H=838$、$L=822$、$C=835$。實體為 1 元，下影線為 12 元，上影線為 3 元；當時適用升降單位為 1 元，所以 $u=1$。整個實體位於上三分之一且下影線通過 $2u$ 門檻，但上影線 $3>u$，因此依本書定義不能命名為錘子形。要稱為相對小或相對長實體，仍須取得同一口徑的前 20 根完成 K 線。條件式解釋：若後續收盤仍留在先定義的關鍵區域上方，且量價關係沒有惡化，才追蹤反彈情境；若收盤跌回區域下方，該情境失效。另一情境是價格只短暫停頓，需以後續無法守住區域確認。行動／風險：沒有價差、深度、失效距離與部位大小時，不因長下影線單獨下單。
...
- **市場資料與公司行動查核**：[TWSE 每日收盤行情](https://www.twse.com.tw/rwd/zh/afterTrading/STOCK_DAY)、[TWSE 除權息計算結果](https://www.twse.com.tw/rwd/zh/exRight/TWT49U)與 [公開資訊觀測站「公告快易查」](https://mopsov.twse.com.tw/mops/web/ezsearch)，查核日期為 2026-08-10。
