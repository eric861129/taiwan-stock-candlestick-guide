...
- 分辨 FOMO、錨定、確認偏誤、損失趨避與結果偏誤的研究脈絡，和教材中可觀察的操作風險。
...
1. **資料有效性**：確認市場、代號、原始價格、決策日與公司行動結果；缺資料就寫出缺口。
2. **時間週期**：寫明使用日 K，並把決策當下可見資料與後續資料分開。
3. **市場狀態**：描述趨勢、波動與流動性，不用情緒詞替代市場背景。
4. **位置**：標出關鍵區域與可用空間，避免以成本價或單一數字當唯一錨點。
5. **K 線／結構**：記錄已完成 K 線與結構，不能拿未出現的結果補理由。
6. **量價關係／流動性**：查核成交量、價差、深度與可成交性；紙上交易不得假設都能成交。
7. **情境／觸發／失效**：先寫交易情境、觸發條件與失效條件，再決定是否可採取行動。
8. **風險／不交易條件**：依部位大小、滑價、成本、執行限制與資料缺口決定放棄交易條件；不得事後移動停損或無規則攤平。
