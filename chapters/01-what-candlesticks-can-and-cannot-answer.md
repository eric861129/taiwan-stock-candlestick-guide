...
![人工日 K 線圖，左側在區間下緣出現收高 K 線，右側在區間上緣出現收高 K 線，說明相同顏色不能直接代表相同結論。](../assets/figures/ch01-concept.svg)

讀法是先說「哪一段時間、哪個位置、收盤相對開盤如何」，再問「若出現哪些後續證據，原本的假設才成立」。圖上沒有任何一個箭頭等於預測。

## 歷史案例

此圖使用臺灣證券交易所（TWSE）2330 的官方原始日資料，範圍為 2024-01-02 至練習決策日 2024-02-20。兩個標記日都收盤高於開盤；圖表不顯示決策日之後的價格，避免用未來資料替當時的判斷找理由。這個區間未列入已查得的公司行動；遇到疑似機械缺口時，仍應先查 [公開資訊觀測站「公告快易查」](https://mopsov.twse.com.tw/mops/web/ezsearch)。
...
![台積電 2330 於 2024 年 1 月 2 日至練習決策日 2 月 20 日的官方原始日線，標示收高 K 線位於不同位置；圖表右端停在決策日，不顯示後續走勢。](../assets/figures/ch01-cases.svg)

這是一張原始價格圖，不是調整後價格圖；官方日資料的來源、期間與查核日已寫在圖表規格。讀者可以重跑資料取得流程，先確認同一根 K 線的 OHLC，再決定要不要提出假設。

## 八步判讀

**八步前置核對。**先確認代號、交易所、日期、原始／調整後價格與缺漏，並確認本圖是日線而非週線；資料或週期不符時，停止本輪判讀。

1. **大方向**：先描述整理、趨勢或事件後波動等可觀察背景，不替市場貼上必漲或必跌標籤。
2. **所在位置**：標出區間下緣、上緣、跳空後或前高附近等可重現的位置。
3. **量價反應**：描述收高、收低、實體、影線與前後 K 線，並比較可取得的量能、成交筆數與買賣價差；量大不自動等於看多。
4. **交易情境**：把「若站穩區間」與「若跌回區間」列為可並存的條件式情境，不先挑選單一故事。
5. **觸發條件**：只有在預先指定的收盤或結構條件出現後，才把情境由觀察改為可追蹤行動。
6. **失效條件**：寫出會推翻每個情境的收盤、區域或資料品質條件。
7. **風險計算**：以觸發價到失效價的每股距離乘上預計部位，另保留成本與滑價；無法估算時不建立部位。
8. **事後檢討**：保存本次資料核對、觀察、情境與未交易理由，之後只檢查流程是否一致，不用後續漲跌改寫當時判讀。
...
- **公司行動機制**：[公開資訊觀測站「公告快易查」](https://mopsov.twse.com.tw/mops/web/ezsearch)，必要時先以公司代號、日期與公告類別查核，再解讀缺口。
