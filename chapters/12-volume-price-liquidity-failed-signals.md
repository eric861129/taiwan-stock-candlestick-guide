...
![人工日 K 線與成交量圖。前段有成交量明顯增加但實體較小的日子，中段價格收盤離開區域但沒有價差與深度資料，右端價格反應與成交量都需要同口徑比較。方向同時以紅色空心、綠色實心、圖例與文字呈現。](../assets/figures/ch12-concept.svg)
...
TWSE [除權息計算結果資料](https://www.twse.com.tw/rwd/zh/exRight/TWT49U?response=json&startDate=20240401&endDate=20240430) 在可視期間內沒有 2330 記錄，故 metadata 的 `corporate_actions` 為空陣列。公司公告仍應由 [公開資訊觀測站「公告快易查」](https://mopsov.twse.com.tw/mops/web/ezsearch)交叉核對；沒有已查得的公司行動，不表示量能或價格一定由單一原因造成。
...
![台積電 2330 於 2024 年 4 月 1 日至決策日 4 月 30 日的官方原始日 K 線與成交量。圖中標示 4 月 12 日至 18 日的五日比較窗、4 月 19 日相對高量與較大價格範圍、4 月 23 日至 24 日的反彈，以及日資料缺乏買賣價差和深度；沒有決策日後資料。](../assets/figures/ch12-cases.svg)
...
**八步前置核對。**確認 TWSE、2330、原始日 K、成交量單位、日期排序、缺漏與公司行動查核；若改用還原價格，價格結果與比較窗保持同一口徑，日成交量只與日成交量比較。

1. **大方向**：分開描述方向、波動與流動性；相對高量可能出現在趨勢、整理、事件或高波動背景。
2. **所在位置**：先標記關鍵區域、缺口或反彈區，再比較同一位置的價格結果和量能。
3. **量價反應**：記錄實體、日內範圍、收盤位置與是否離開區域，宣告 \(W\) 的日期與統計量並排除目標日；另列價差、深度、成交頻率、滑價和未知欄位。
4. **交易情境**：把量價條件達標的延續或反轉情境，和資料不足的等待情境分開記錄。
5. **觸發條件**：若計畫要求量能或可成交性門檻，觸發前必須實際取得並核對它。
6. **失效條件**：收盤回區、量價條件未達、比較窗不可重現或報價資料缺失，都使該情境無效。
7. **風險計算**：把預計部位造成的滑價、無法按失效條件執行的可能性與成本納入最大損失；關鍵流動性資訊缺失時不交易。
8. **事後檢討**：保存 \(W\) 的組成、資料缺口與實際成交差異，回顧時檢查是否把量柱誤寫成價格形狀已完成的證明。
