...
![人工日 K 線圖以區域、箭頭與標籤示範突破前的多個兼容情境；觸發、失效與放棄條件分開標示，沒有把任何分支畫成預測答案。](../assets/figures/ch15-concept.svg)
...
![台積電 2330 於 2024 年 5 月 1 日至 5 月 31 日的官方原始日線，標出可能的突破區、失效邊界與未查明的流動性條件；圖表終點是決策日。](../assets/figures/ch15-cases.svg)
...
**八步前置核對。**先確認決策日、原始價格、公司行動與資料完整，標記使用日線或其他週期；未完成 K 線不能當成觸發。

1. **大方向**：描述方向、波動與流動性，不先選擇多方或空方故事。
2. **所在位置**：畫出區域上下緣、前高前低與可用價格空間。
3. **量價反應**：定義收盤、影線與相鄰高低點的可觀察條件，並列出量能比較窗口、價差與深度是否可得。
4. **交易情境**：每個分支都寫七欄，觸發前允許多個情境並存。
5. **觸發條件**：把每一分支的收盤、量價或可成交性條件寫成事前可檢查的行動門檻。
6. **失效條件**：把推翻每一分支的區域、結構、資料或執行條件獨立寫出，不和觸發混在一起。
7. **風險計算**：用觸發到失效的距離、預計部位、成本與滑價計算或估計最大損失；條件不利時明確不交易。
8. **事後檢討**：保存決策日各分支及其未觸發原因，回顧時檢查是否事後刪除了原本存在的情境。
