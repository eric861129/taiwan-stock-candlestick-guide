...
本章回答：「我知道進場價了，該買幾股、停損放哪裡，這筆交易值得做嗎？」答案必須從情境失效點與可執行限制推導，而不是先挑一個百分比再找圖形理由。你會計算每股風險、貨幣風險、整股取整、剩餘風險、R 倍數與期望值，並把費用、稅、價差、滑價與成交不確定性分開。
...
其中 (u) 是市場允許的最小股數或交易單位；現行台股規則與整股／零股限制請回看[附錄 C](appendix-c-taiwan-market-rules.md)，不要把會變動的規則複製到本章。取整後：

$$
R_{residual}=q_{exec}\times r_{unit}
$$

若 (q_{exec}=0) 或 (R_{residual}>R_{plan})（例如採四捨五入），就不執行；本章一律向下取整以免放大計畫風險。
...
$$
R_{realized}=\frac{PnL_{net}}{R_{residual}}
$$
...
以下只是單位與取整示範，不是風險比例建議：假設多方 (P_{entry}=100) 元、情境失效 (P_{inv}=96.5) 元、計畫貨幣風險 (R_{plan}=600) 元、最小單位 (u=1) 股。則 (r_{unit}=3.5) 元，(q_{raw}=600/3.5=171.428571...)，向下取整 (q_{exec}=171) 股，(R_{residual}=171\times3.5=598.5) 元。若實際出場 108 元，毛損益 (=(108-100)\times171=1,368) 元；淨損益還要扣以符號表示的費用、稅與滑價。這個例子沒有宣稱任何人應承擔 600 元風險。
...
<!-- figure-spec
{
  ...
  "annotations":[
    ...
    {"type":"line","start":"2024-01-10","end":"2024-01-17","start_price":100,"end_price":100,"label":"失效價：先於股數"},
...
以 TWSE 2330 於 2024-06-03 至 2024-06-28 的官方原始日資料示範「風險太大就不交易」，決策日為 6 月 28 日。資料來自 [TWSE 每日收盤行情](https://www.twse.com.tw/rwd/zh/afterTrading/STOCK_DAY)，公司行動查核 [TWSE 除權除息](https://www.twse.com.tw/rwd/zh/exRight/TWT49U) 與 [MOPS](https://mops.twse.com.tw/mops/web/t05st01)；圖表不暗示成交或獲利。
...
<!-- figure-spec
{
  ...
  "annotations":[
    {"type":"zone","start":"2024-06-03","end":"2024-06-14","low":815,"high":875,"label":"進場假設區；先找失效"},
    {"type":"line","start":"2024-06-14","end":"2024-06-28","start_price":810,"end_price":810,"label":"結構失效邊界"},
    {"type":"label","date":"2024-06-21","price":900,"label":"可用空間與成本仍待估"},
    {"type":"zone","start":"2024-06-24","end":"2024-06-28","low":810,"high":900,"label":"風險若無法容納則放棄"}
  ]
}
-->
...
1. **資料有效性**：核對原始價格、決策日、公司行動與成交資料。
2. **時間週期**：決定使用日線或其他週期，記錄 ATR 暖機。
3. **市場狀態**：描述方向、波動、流動性，不能以風險公式補足缺資料。
4. **位置**：找結構失效、前方障礙與可用空間。
5. **K 線／結構**：定義進場與失效的收盤／高低點條件。
6. **成交量／流動性**：確認交易單位、價差、深度與可能滑價。
7. **情境、觸發、失效**：先寫計畫風險，再決定是否觸發。
8. **風險／放棄交易**：取整後仍超限、成本未知或空間不足就不交易。

## 練習

使用本章假設例，完成以下工作表：

1. 列出 (P_{entry})、(P_{inv})、(r_{unit})、(R_{plan})、(q_{raw})、(q_{exec})、(R_{residual}) 與單位。
2. 假設實際出場 98 元，先算毛損益，再用符號 (f_{buy}, f_{sell}, t, Slippage) 寫淨損益，不得偷填現行費率。
3. 若最小可交易單位改為 100 股，判斷是否仍能把風險放在 600 元內，並寫出放棄條件。

<details>
<summary>參考答案與評分方向</summary>

每個欄位與單位正確得 4 分；向下取整與剩餘風險得 2 分；毛／淨損益分離得 2 分；能判斷 100 股單位下的最小部位與放棄條件得 2 分。不要以任何勝率或固定百分比替代計算。
</details>

## 答案與評分

滿分 10 分：風險公式 3 分、取整 2 分、成本與 R 2 分、期望值單位 1 分、放棄交易 2 分。把停損先定成固定百分比再找圖形理由，視為流程錯誤。
