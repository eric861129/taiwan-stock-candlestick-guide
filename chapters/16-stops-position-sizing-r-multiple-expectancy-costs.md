...
6. **量價關係／流動性**：確認交易單位、價差、深度與可能滑價。
7. **情境、觸發、失效**：先寫計畫風險，再決定是否觸發。
8. **風險／不交易條件**：取整後仍超限、成本未知或空間不足就不交易。
...
假設進場 100 元、失效 96.5 元、計畫風險 600 元且 u=1 股：每股風險 3.5 元，未取整股數約 171.428 股，執行 171 股，取整後風險 598.5 元。若實際出場 98 元，毛損益為 (98-100)×171=-342 元；淨損益為 -342-C_buy-C_sell-Tax-Slippage，費率未知時不得代入現行數字。若 u=100 股，執行 100 股、取整後風險 350 元，仍在 600 元內；若 u=200 股，最小部位的風險為 200×3.5=700 元，已超過計畫風險，依向下取整公式 q_exec=0，因此不交易。實務上還要把實際成交與成本補入紀錄。
