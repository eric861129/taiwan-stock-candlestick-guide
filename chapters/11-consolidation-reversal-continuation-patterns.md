...
![人工日 K 線圖。左側以多次反應標出 100 到 106 的區間，接著價格短暫收盤離開後返回區間，右側的高低點逐步收斂但停在決策右端，說明假突破與三角形候選都需預先定義邊界。紅色空心、綠色實心與文字共同表示方向。](../assets/figures/ch11-concept.svg)
...
公司行動以 TWSE [除權息計算結果資料](https://www.twse.com.tw/rwd/zh/exRight/TWT49U?response=json&startDate=20240501&endDate=20240612) 查核。該查詢在圖表期間內沒有 2330 記錄，因此 metadata 使用空陣列；[公開資訊觀測站「公告快易查」](https://mopsov.twse.com.tw/mops/web/ezsearch)保留為公司公告交叉查核來源。這個檢查不為區間或突破指定成交者的原因。
...
![台積電 2330 於 2024 年 5 月 2 日至決策日 6 月 5 日的官方原始日 K 線與成交量。圖中標示 5 月中旬的反覆反應區、5 月 22 日收盤離開、5 月 30 日回到區域，以及 6 月 5 日右端的邊界選擇不確定性；沒有顯示決策日後走勢。](../assets/figures/ch11-cases.svg)
...
**八步前置核對。**確認 TWSE、2330、原始日 K、日期排序、缺值與公司行動查核；固定日 K 與 `k=1` 錨點，較長週期邊界要以其自身資料重畫。

1. **大方向**：分開描述方向、波動與流動性，確認此處是整理、趨勢拉回還是轉換候選。
2. **所在位置**：以反覆反應畫出關鍵區域，寫明每一側邊界由哪些日期與 OHLC 支持。
3. **量價反應**：檢查區域兩側是否有足夠錨點、收盤有沒有離開或回收，並依同標的、同週期比較窗檢視量能；沒有價差和深度資料時不下流動性定論。
4. **交易情境**：至少列出守住區域與回到區域兩種情境，不以量度幅度當成必到目標。
5. **觸發條件**：每個情境都以可重做的收盤、區域或結構條件觸發。
6. **失效條件**：收盤回收、錨點不足、邊界無法重現或可成交性惡化時，對應情境失效。
7. **風險計算**：以觸發點到區域另一側失效點的距離、預計部位和成本估損失；邊界太寬或距離過大時不交易。
8. **事後檢討**：保存區域錨點、選擇的條件和放棄理由，回顧時檢查型態名稱是否曾依賴任意延伸。
