...
![人工日 K 線圖以五期 SMA、五期 EMA 與五期 ATR 標示平滑與波動；下方成交量先放大後回落，說明均線有延遲、ATR 沒有方向，均量也不能單獨證明突破。](../assets/figures/ch13-concept.svg)
...
![台積電 2330 於 2024 年 1 月 2 日至 3 月 29 日的官方原始日線，顯示均線暖機、成交量窗口與 ATR 擴大；圖表只到決策日，不含其後資料。](../assets/figures/ch13-cases.svg)
...
**八步前置核對。**確認 1d、原始價格、日期連續與公司行動查核完成；標記均線、均量與 ATR 的期間，說明暖機是否足夠。

1. **大方向**：分開描述方向、波動與流動性，不把 ATR 當成方向訊號。
2. **所在位置**：找前高、前低、關鍵區域與可用價格空間，並標示均線相對位置。
3. **量價反應**：說明收盤位置、影線與均線距離，用同標的同期間均量比較，並保留價差與成交深度是否未知。
4. **交易情境**：例如把「收盤突破區域後延續」與「仍在區間內等待」分開，不讓均線交叉獨自決定行動。
5. **觸發條件**：只有完成 K 線收盤突破預定區域，且所用指標已通過暖機與量價核對時才觸發。
6. **失效條件**：收盤回到區域內、均線或 ATR 資料不足，或價格空間不再支持原情境時失效。
7. **風險計算**：由結構失效點推導每股風險，必要時把 ATR 作為波動描述，再檢查部位與成本是否在上限內。
8. **事後檢討**：保存指標設定、暖機資料、觸發與失效紀錄，回顧時檢查是否把 ATR 或均線當成保證方向。
