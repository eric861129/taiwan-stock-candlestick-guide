...
1. **資料有效性：**確認交易日順序、缺值、停牌與原始／還原價格模式；若附近有公司行動，先核對公告。
2. **時間週期：**寫下本次以日線判讀；若週線背景不同，將它另列為較長時間週期的假設。
3. **市場狀態：**描述目前是整理、擴張、趨勢延伸或方向不明，不把標籤當成預測。
4. **位置：**先畫出有重複反應的區域，再定義價格從哪一側離開與回測。
5. **K 線／結構：**比較缺口兩側的高低點、收盤是否越界，以及回到區域時的收盤位置。
6. **量能／流動性：**檢查成交量是否與自己的比較窗相符；日成交量不能單獨證明流動性充足。
7. **情境、觸發與失效：**只有在重新離開且收盤守住時考慮觸發；收盤回到區間內是失效訊號之一。
8. **風險／不交易：**若缺口與區域定義不同步、公司行動尚未核對，或風險距離無法接受，就記錄為不交易。
