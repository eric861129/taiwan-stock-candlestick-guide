...
完成本章後，你能把較長時間週期當成背景、把主要交易週期當成執行資訊，並用三個可分開檢查的面向描述市場狀態：方向、波動與流動性。你也能辨認資訊不足時應保留為不交易，而非硬替市場貼標籤。

## 先說結論

- 多時間週期判讀的重點在於先後順序：先看較長週期的結構背景，再用主要交易週期定義位置、觸發與失效。
...
**八步前置核對。**確認日線資料完整、價格模式為原始價格，並查核決策日附近公司行動與休市日；先指定較長週期背景，寫明日線用於執行或等待，未完成週不與完整週混比。
