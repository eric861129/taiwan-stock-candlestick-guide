...
1. **圖表與資料有效性**：確認市場、代號、日期、成交量欄位和單位；上櫃資料先確認是否已由成交仟股標準化為股數。
2. **週期**：日成交量不能直接和週成交量或盤中量柱比較，先統一週期。
3. **市場狀態**：描述量能在整理、突破嘗試、事件日或趨勢中的相對位置。
4. **位置**：把價格放回可重現的區間、前高前低或缺口位置，再看量能是否有變化。
5. **K 線／結構**：先描述 OHLC 與連續結構；量柱不替代價格結構。
6. **成交量／流動性**：記錄成交股數、平均量口徑、買賣價差、深度、滑價和缺漏；只具其中一項時要降低結論。
7. **情境／觸發／失效**：例如「若價格突破且成交／可成交性資料符合預先條件才追蹤」；若量能或報價條件未達標，情境不成立。
8. **風險／不交易**：預計下單量可能造成滑價、無法停損或超出可承受損失時，減少部位或不交易。
