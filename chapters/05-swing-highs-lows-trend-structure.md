...
![人工日 K 線圖，前段呈現較高波峰和較高波谷，中段出現一般拉回，後段跌破較高波谷並停在尚未確認的右端，說明結構轉換仍需後續資料。](../assets/figures/ch05-concept.svg)
...
![台積電 2330 於 2024 年 5 月 2 日至練習決策日 6 月 12 日的官方原始日線。圖中標示較高波峰與較高波谷序列、5 月底的結構破壞，以及右端尚不能確認的新波峰；圖表不含決策日後資料。](../assets/figures/ch05-cases.svg)
...
**八步前置核對。**確認 TWSE、2330、日線、raw 價格、日期排序與公司行動紀錄；本例可視期間無除權息紀錄，較長背景須另以完整週線聚合。

1. **大方向**：5 月中旬可描述為上升結構；5 月底到 6 月初則降級為結構受破壞後的轉換候選。
2. **所在位置**：把 5/31 放回前一個較高波谷與最近反彈區，而非只看單日跌幅。
3. **量價反應**：固定 `k=1` 確認局部波峰、波谷，再比較序列是否仍較高或較低；日成交量只作相對線索，沒有價差、深度或預計下單量時不把它當成可執行性證明。
4. **交易情境**：情境一是反彈後形成新的較高波谷；情境二是回到結構破壞區下方，兩者都可在確認前並存。
5. **觸發條件**：各情境必須先以收盤與區域條件確認新的波谷或再次跌破，才採取下一步。
6. **失效條件**：已確認的新波谷被收盤跌破，或價格無法離開結構破壞區時，原先的反彈情境失效。
7. **風險計算**：以觸發價到對應結構失效區的距離估每股風險；失效範圍過寬或部位無法壓在風險上限內時不交易。
8. **事後檢討**：保存 `k=1` 錨點、當日可見的波峰波谷與未交易原因，回顧時不可用右側反彈替未確認結構加分。
