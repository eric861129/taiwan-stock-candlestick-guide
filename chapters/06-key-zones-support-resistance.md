...
1. **圖表與資料有效性**：確認市場、代號、日線、raw 價格、日期與公司行動篩查結果。
2. **週期**：日線區域不能直接套成週線區域；較長週期的區域要用相同週期資料重建。
3. **市場狀態**：先分辨價格是在趨勢中拉回、整理中來回，或結構受破壞後的轉換。
4. **位置**：用反覆反應與整理帶畫成範圍，記錄價格在區內、區外或穿越中的哪一種位置。
5. **K 線／結構**：比較收盤是否留在區外、是否出現拒絕或重新穿越，不能只看盤中最高或最低。
6. **成交量／流動性**：量能可與同一回看窗比較；若沒有買賣價差、深度與預計下單量，流動性結論必須保留。
7. **情境／觸發／失效**：情境可包含守住區域、跌回區內、再度穿越；觸發與失效都要用收盤或結構條件寫清楚。
8. **風險／不交易**：區域太寬、觸發離失效點太遠、資料不足或可成交性不明時，避免用精確線硬湊進場理由。
...
- **公司行動查核**：[TWSE 除權息計算結果表](https://www.twse.com.tw/zh/announcement/ex-right/twt49u.html) 與 [MOPS 除權息公告](https://mops.twse.com.tw/mops/web/t05st01)。
